 ...
 * @return void
 **/
void network_manager(struct network_data* nm, char* host_ip, uint16_t host_port)
{
	nm->dest_addr.sin_addr.s_addr = inet_addr(host_ip);
	nm->dest_addr.sin_family = AF_INET;
	nm->dest_addr.sin_port = htons(host_port);
	...
	{
		ESP_LOGI(TAG, "Socket created, connected to %s:%d", host_ip, host_port);
	}
}
