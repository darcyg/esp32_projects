...
/*
        // setup magnetometer
    ESP_LOGI(TAG, "MPU setting up magnetometer...");
#ifdef CONFIG_MPU_AK89xx
    if (MPU_ERR_CHECK(compassInit())) return err;
#ifdef CONFIG_MPU_AK8963
    ESP_LOGI(TAG, "MPU setting up magnetometer sensitivity...");
 ...
 * @brief Read data contained in FIFO buffer.
 * */
esp_err_t MPU::readFIFO_HS(size_t length, uint8_t* data)
{
    return MPU_ERR_CHECK(readBytesHS(regs::FIFO_R_W, length, data));
}
