...
import readSensor_naive_WiFi as rs
import numpy as np
...
class ProjectionViewer:
    ...
    def run(self, sensorInstance):
        ...
        while running:
            ...
            data = sensorInstance.getData()
            self.wireframe.quaternion.q = np.array((data[0], data[1], data[2], data[3]))
            self.display()
            pygame.display.flip()
...
if __name__ == '__main__':
    ...
    w = rs.WifiRead(server_address, port, dataNumBytes, numParams)  # initializes all required variables
    w.readSerialStart()  # starts background thread
    block = initializeCube()
    pv = ProjectionViewer(1920, 1080, block)
    pv.run(w)
